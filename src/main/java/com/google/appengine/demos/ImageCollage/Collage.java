...
public class Collage {
    ...
    public Collage(Image inputImg, int maxDepth, int varianceThreshhold, int factor, Crawler crawler, int initialX, int initialY, CollageTimer collageTimer, HashMap<String, ProcessedImage> imageCache){
        setParameters(inputImg, maxDepth, varianceThreshhold, factor, crawler, initialX, initialY, collageTimer, imageCache);
    ...
    Input: alpha: Opacity of the original image to be overlaid on the collage

     */

    public Collage(Image inputImg, int maxDepth, int varianceThreshhold, int factor, Crawler crawler, int initialX, int initialY, double alpha, CollageTimer collageTimer, HashMap<String, ProcessedImage> imageCache){
    ...
    private void setParameters(Image inputImg, int maxDepth, int varianceThreshhold, int factor, Crawler crawler, int initialX, int initialY, CollageTimer collageTimer, HashMap<String, ProcessedImage> imageCache){

        this.factor = factor;
        this.crawler = crawler;
        processedImage = new ProcessedImage(inputImg, null, null, null, null);
    ...
    public Image getCollage(boolean smartSize){
        if(smartSize){
            return alphaOverlay(colorBlock(0, 0, height, width, 0, processedImage,0));
        }
        else{
    ...
    private Image colorBlock(int firstX, int firstY, int partitionHeight, int partitionWidth, int depth, ProcessedImage colorMe, double weight){

        if(firstX<width && firstY<height) { // Prevents colorBlock() from creating a collage outside of the given base-image
            if (depth < maxDepth && !collageTimer.timeElapsed(weight)) { // Prevents colorBlock() from recursing past the maxDepth and cuts execution at the set Time
                ...
                if (variance > varianceThreshhold){ // Stops the quad-tree from recursing if the variance is below the threshhold
                    ...
                    Image upperLeft = colorBlock(firstX, firstY, halfHeight, halfWidth, depth, colorMe.getBlock(0, 0, halfHeight, halfWidth),weight+depth*8); // Upper Left, 12, 8
                    Image upperRight = colorBlock(firstX + halfWidth, firstY, halfHeight, partitionWidth - halfWidth, depth, colorMe.getBlock(halfWidth, 0, halfHeight, partitionWidth - halfWidth),weight+depth*4); // Upper Right, 6, 4
                    Image lowerLeft = colorBlock(firstX, firstY + halfHeight, partitionHeight-halfHeight, halfWidth, depth, colorMe.getBlock(0, halfHeight, partitionHeight-halfHeight, halfWidth),weight); // Lower Left, 0
                    Image lowerRight = colorBlock(firstX + halfWidth, firstY + halfHeight, partitionHeight-halfHeight, partitionWidth - halfWidth, depth, colorMe.getBlock(halfWidth, halfHeight, partitionHeight-halfHeight, partitionWidth - halfWidth),weight-depth*1.5 ); // Lower Right, -3, -2
    ...
    private Image colorBlock(int firstX, int firstY, int partitionHeight, int partitionWidth, int depth, ProcessedImage colorMe){
        ...
        if (depth < maxDepth) { // Prevents colorBlock() from recursing past the maxDepth and cuts execution at the set Time
            ...
            Image upperRight = colorBlock(firstX + halfWidth, firstY, halfHeight, partitionWidth - halfWidth, depth, colorMe.getBlock(halfWidth, 0, halfHeight, partitionWidth - halfWidth)); // Upper Right, 6, 4
            Image lowerLeft = colorBlock(firstX, firstY + halfHeight, partitionHeight - halfHeight, halfWidth, depth, colorMe.getBlock(0, halfHeight, partitionHeight - halfHeight, halfWidth)); // Lower Left, 0
            Image lowerRight = colorBlock(firstX + halfWidth, firstY + halfHeight, partitionHeight - halfHeight, partitionWidth - halfWidth, depth, colorMe.getBlock(halfWidth, halfHeight, partitionHeight - halfHeight, partitionWidth - halfWidth)); // Lower Right, -3, -2
    ...
    private Image findImage( ProcessedImage block, int firstX, int firstY, int partitionHeight, int partitionWidth, int depth){
        ...
        attributionTable.add(new AttributionCell(firstX+initialX,firstY+initialY,firstX+initialX+partitionWidth,
                firstY+initialY+partitionHeight,processed.getUsername(),processed.getUrl(),processed.getId(), processed.getTitle()));

        // Tracks the location, author, and Url of the Flickr image within the Collage

        return processed.getScaled(partitionWidth*factor,partitionHeight*factor,depth);
    ...
    */
    ...
    public class AttributionCell{
        ...
        */
        ...
        public AttributionCell(double x1, double y1, double x2, double y2, String author, String url, String id, String title){
            this.x1 = x1;
            this.x2 = x2;
            this.y1 = y1;
            this.y2 = y2;
