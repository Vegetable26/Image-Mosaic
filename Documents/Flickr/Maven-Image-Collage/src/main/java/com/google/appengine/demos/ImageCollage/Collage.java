...
public class Collage implements Runnable{
    ...
    public Collage(Image inputImg, int maxDepth, int varianceThreshhold, int factor, Crawler crawler, int initialX, int initialY, CollageTimer collageTimer, HashMap<String, ProcessedImage> imageCache, ThreadFactory tf, boolean smartSize) {
        this.factor = factor;
        ...
        System.out.println("this thread has"+myTime/Math.pow(10,9)+','+System.nanoTime()/Math.pow(10,9));
    ...
    */
    ...
    private Image alphaOverlay(Image collage){
        ...
        return transformer.composite(composites,width*factor,height*factor,0,0); // Returns the overlaid collage and alpha% opacity base-image

    }
    public Image getCollage(){return alphaOverlay(transformer.composite(totalImage, width * factor, height * factor, 0,0));}
    ...
    */
    ...
    Non-functioning threading: produces 64 threads, 14 over the Google limit

     */

    private class biCollage implements Runnable{
        private int x1,x2,y1,y2;
        private boolean smartSize;
        public biCollage(boolean smartSize, int x1, int x2, int y1, int y2){
            this.x1 = x1;
            this.x2 = x2;
            this.y1 = y1;
            this.y2 = y2;
        ...
        public void run(){
            int range = (int)(Math.floor(height)/2);
            createCollage(smartSize, x1, x2, y1, y1 + range);
            createCollage(smartSize, x1, x2, y2 - range, y2);
        }
    }
    public void run(){
